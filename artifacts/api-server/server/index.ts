...
app.use((req, res, next) => {
  ...
  if (origin && nativeAllowedOrigins.has(origin)) {
    ...
    res.setHeader("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Requested-With, X-StillHere-Native");
...
(async () => {
  ...
  // REPLIT: When DISABLE_FRONTEND=1 the API runs standalone (Vite dev server
  // is a separate process in the Replit monorepo).  Skip both static serving
  // and the Vite middleware so the server starts cleanly as an API-only process.
