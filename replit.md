...
- **Learning Mode (Safe-Start Guard):** New accounts get 30-day `learningModeUntil` timestamp. During learning mode, the system only uses missed check-in, SOS, crash detection, and hard heartbeat silence. No pattern-based concern language ("unusual for you," routine deviation) is used. Watcher cards show "Learning" badge. Home screen shows learning mode card explaining the system is getting to know the user's routine.
- **Sleep Protection:** User-configurable `sleepStart`/`sleepEnd` (default 22:30-07:00). During sleep hours, non-emergency concern notifications (missed check-in, heartbeat silence) are suppressed. SOS and crash detection always fire. `isUserInSleepHours()` function in notification engine handles timezone-aware sleep window calculation.
- **Your Protection Panel:** Home screen panel showing who is in the Safety Circle, what they can currently see, and the current sharing mode. Controls for Precise/Area/Presence/Paused sharing modes. Shows sleep protection hours and "You decide what they can see." line.
- **See My Guardian's View:** One-tap preview on home screen showing exactly what watchers currently see (safety state, location visibility based on sharing mode, battery, last check-in). Synchronized mirror, not a mockup.
- **Presence-Only Mode:** `sharingMode` enum (`precise`/`area`/`presence`/`paused`) on users table. When mode is `presence` or `paused` and safety state is not `concern`, the `getWatchedUsers` API strips lat/lng from the response. During concern state, location auto-unlocks so the system can help. API endpoint: `POST /api/sharing-mode`.
- **Adaptive Heartbeat:** Client-side heartbeat adjusts interval based on battery: critical (<10%) = 5min, low (<30%) = 2min, normal/charging = 1min. Watcher battery badge shows context ("Updates reduced to save battery"). Server endpoint `GET /api/heartbeat/recommended-interval` returns recommended interval.
- **Setup Confirmation Handshake:** When contacts are first saved, protected user gets push: "You're protected. [Name] is now watching over you." Watcher gets guardian briefing: "You've been added as [Name]'s Primary Guardian. If we can't reach them, we'll guide you. You won't need to figure anything out."
- **Safety Circle Roles:** `circleRole` enum (`primary`/`backup`/`support`) on contacts table. Primary = first person contacted, Backup = next if Primary unavailable, Support = kept informed if serious. Roles shown as badges on watcher cards and settings page. API endpoint: `POST /api/contacts/:contactId/role`.
- **Incident Claim Flow ("I've got this"):** `claimedByContactId` and `claimedAt` on incidents table. When a watcher claims an incident, all other watchers get push notification: "[Name] is handling this now. No action needed from you." Real-time Socket.IO `incident:claimed` event. API endpoint: `POST /api/incidents/:incidentId/claim`.
- **Safety Circle Dry Run:** Test feature at `POST /api/safety-drill`. Creates a test incident with `isDrill=true`, sends drill notification to all watchers, auto-resolves after 60 seconds. Clearly labeled as a drill. Cannot run while a real incident is open.
- **Action-First Incident Language:** All concern notifications now include a clear action or instruction to wait. Examples: "We haven't heard from Sarah. Tap here to try reaching them." / "We're still trying to reach Sarah. No action needed yet."
