...
- **Driving Safety:** A dedicated driving dashboard includes a speedometer, live route map, stats, and crash detection with a 60-second auto-SOS countdown. It also generates Life360-style driving habit reports for users and emergency contacts.
...
- **Live Location Sharing:** Real-time GPS location sharing with adaptive update frequency, activity detection, and "Open in Google Maps" integration. Includes robust background persistence mechanisms and native Capacitor plugin support.
- **Geofencing:** Allows creation of named geofences with real-time zone departure detection and email alerts to contacts.
- **Location Breadcrumbs:** Stores a trail of location points during active sessions, accessible to watchers.
