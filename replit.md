...
- **Premium Chat Quick Actions:** Three classy gradient pill buttons (icon-on-top + label) for Share Location (primary blue), Call (emerald), and SOS (destructive red with two-tap confirm). Each has hover elevation, active-press scale, and per-button colored backgrounds rather than plain outlined buttons.
