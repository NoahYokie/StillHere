...
- **Check-in Mechanism:** Manual or optional automatic check-ins, with configurable schedules, grace periods, and reminders. SMS checkin supported (reply YES to reminder text). Location-aware checkins capture lat/lng and auto-detect timezone via browser Geolocation API + `Intl.DateTimeFormat`. User's timezone auto-updates when they travel. The 24-hour schedule is UTC-based so it persists correctly across timezone changes. Checkins table stores `lat`, `lng`, `timezone`.
...
- **Communication:** Socket.IO enables real-time in-app messaging with persistence and read receipts, and WebRTC voice calling with TURN relay via Twilio NTS. Native call integration via Capacitor plugins (CallKit/ConnectionService) provides a native incoming call UI. Instagram-style message inbox (`/inbox`) with conversation list, unread counts per thread, and real-time updates via socket. Red unread badge on home header message button. Slide-in notification banner for new messages. Full-screen overlay with ringtone for incoming calls. Voice call UI shows avatar, call duration timer, mute toggle. Video calling is planned as a future upgrade on top of the voice infrastructure. **Messaging uses socket-first sending** with optimistic UI updates (instant display), typing indicators (bouncing dots), and retry on failure. All communication is encrypted via HTTPS/WSS transport security. API: `GET /api/conversations`, `GET /api/messages/unread/count`.
- **Driving Safety:** GPS-based speed monitoring and accelerometer crash detection. Configurable speed limit alerts (40-120 km/h). Crash detection triggers a 60-second countdown with auto-SOS. Drive sessions track max/avg speed, distance, and crash events. All contacts notified via SMS, push, and email on crash detection. DB tables: `drive_sessions`, `speed_alerts`. Settings: `drivingSafety` (toggle), `speedLimitKmh`. API: `POST /api/drive/start`, `POST /api/drive/end`, `GET /api/drive/active`, `POST /api/drive/speed`, `POST /api/drive/crash`, `GET /api/drive/history`, `GET /api/drive/alerts`.
- **Watcher System:** Automatic detection of emergency contacts who are also StillHere users, enabling push notifications and an enhanced watcher dashboard with grouped status sections (active alerts, overdue, all clear), quick actions (message/call), and next-due-time display. Watchers can opt out (stop watching) via confirmation dialog; soft-deleted for 30 days with restore option. Owner notified via push notification. Removed contacts shown on both watcher dashboard and owner's settings page. Contact tokens revoked on opt-out for privacy. Restore enforces contact limits and priority conflicts. Cron cleans up expired soft-deletes after 30 days. API: `POST /api/watched-users/:contactId/opt-out`, `GET /api/watched-users/removed`, `POST /api/watched-users/:contactId/restore`, `GET /api/contacts/removed`, `POST /api/contacts/:contactId/restore`.
- **Live Location Sharing:** Real-time GPS location sharing with emergency contacts. Adaptive update frequency (5s when moving, 30s when stationary). Automatic activity detection (stationary/walking/running/cycling/driving) based on GPS speed. Duration-based or open-ended sharing sessions. Watchers see live position, activity, speed, heading, and location trail in real time via Socket.IO. "Open in Google Maps" for pinpoint navigation. **Background persistence:** Silent audio keep-alive prevents browser suspension, Wake Lock API keeps screen active, drift-detection keep-alive timer auto-restarts GPS on suspension, `visibilitychange`+`focus` auto-resume, service worker periodic sync, server-side cron sends push notification to wake app when GPS stale >2min. localStorage `liveLocationActive` flag persists tracking intent across app restarts. Native Capacitor plugin (`@transistorsoft/capacitor-background-geolocation`) for true background tracking in native builds. DB tables: `live_location_shares`, `live_location_points`. API: `POST /api/live-location/start`, `POST /api/live-location/stop`, `GET /api/live-location/status`, `POST /api/live-location/update`, `GET /api/live-location/watching`, `GET /api/live-location/trail/:userId`. Pages: `/live-location`, `/live-location/:userId`.
...
- **Error Tracking:** Automatic crash/error reporting via global error boundary and unhandled exception listeners. Frontend errors (component crashes, uncaught errors, unhandled promise rejections) are captured and sent to the server. User-scoped error view on `/feedback` page. DB table: `error_reports`. API: `POST /api/errors/report` (unauthenticated), `GET /api/errors`, `GET /api/errors/stats`, `POST /api/errors/:id/resolve`.
- **App Ratings:** In-app 1-5 star rating prompt that appears after 5 checkins. Users can update their rating anytime. Feedback page shows overall rating stats with distribution chart and anonymized recent reviews. Dismissible prompt with 7-day cooldown. DB table: `app_ratings`. API: `POST /api/ratings`, `GET /api/ratings/mine`, `GET /api/ratings`, `GET /api/ratings/stats`.
